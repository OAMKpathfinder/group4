...
async function housesValidate(req, res, next){
    try{
        ...
        const schema = Joi.object({
            decade: Joi.number().integer().max(currentYear).required(),
            ...
            UsersId: Joi.number().integer().min(0).required(), 

            LocationsId: Joi.number().integer().min(0).required(),

            HeatingSystemsId: Joi.number().integer().min(0).required()
        });
